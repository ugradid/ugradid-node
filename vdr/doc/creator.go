...
// DefaultCreationOptions returns the default DIDCreationOptions:
// no controllers, CapabilityInvocation = true,
// AssertionMethod = true and SelfControl = true
...
func didSubKIDNamingFunc(owningDID did.DID) ucrypto.KIDNamingFunc {
...
func (n Creator) Create(options vdr.DIDCreationOptions) (*did.Document, ucrypto.Key, error) {
	...
	if options.SelfControl {
		...
		key, err = ucrypto.NewEphemeralKey(didKIDNamingFunc)
	}
	if err != nil {
		return nil, nil, err
	}
