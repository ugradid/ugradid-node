...
import (
	...
	"github.com/ugradid/ugradid-node/network/transport/v1/logic"
...
func New(config Config, adapterConfig p2p.AdapterConfig, graph dag.Dag, publisher dag.Publisher, payloadStore dag.PayloadStore) transport.Protocol {
	adapter := p2p.NewAdapter()
	return &protocolV1{
		config:        config,
		adapterConfig: adapterConfig,
		adapter:       adapter,
		protocol:      logic.NewProtocol(adapter, graph, publisher, payloadStore),
	}
}
